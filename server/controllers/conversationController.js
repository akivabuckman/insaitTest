...
export const _addConversation = async (req, res) => {
    try {
        ...
        const clientsResponse = await fetch(`${process.env.REACT_APP_ADDRESS}/clients/getClients`);
...
export const _populateConversations = async (req, res) => {
    ...
    try {
        ...
        for (let chatIndex = 0; chatIndex < quantity; chatIndex++) {
            const response = await fetch(`${process.env.REACT_APP_ADDRESS}/conversations/addConversation`, {
                method: "POST",
